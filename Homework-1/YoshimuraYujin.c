...
int main(int argc, char **argv) {
  ...
  frequency = create_array(length);
  initialize_array(frequency, length);
  ...
  for (i = 0; i < ITERATION; i++) {
    ...
    for (itr = 0; itr < length; itr++) {
      ...
      // Sum up global sample
      for (j = 0; j < global_size; j++) {
        frequency[itr] = add(frequency[itr], global_sample[j]);
      }
    }
    ...
    if (i == 0) {
      ...
      print_sample(frequency, global_size);
    }
