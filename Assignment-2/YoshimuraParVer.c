...
// mpicc YoshimuraParVer.c -o YoshimuraParVer_Exe
// ***************************************************************************
...
int main(int argc, char **argv) {
  ...
  local_size = global_size / comm_sz;
  local_sample = create_array(local_size);

  // Creates an MPI type for struct complex
  const int count = 3;
  int blocklength[3] = {1, 1, 1};
  MPI_Aint offsets[3];
  MPI_Datatype types[3] = {MPI_INT, MPI_FLOAT, MPI_FLOAT};
  MPI_Datatype mpi_complex;
  offsets[0] = offsetof(Complex, id);
  offsets[1] = offsetof(Complex, re);
  offsets[2] = offsetof(Complex, im);
  MPI_Type_create_struct(count, blocklength, offsets, types, &mpi_complex);
  ...
  for (i = 0; i < ITERATION; i++) {
    ...
    for (int itr = 0; itr < length; itr++) {
      // Scatters the values to each process
      MPI_Scatter(global_sample, local_size, mpi_complex, local_sample, local_size, mpi_complex, 0, MPI_COMM_WORLD);

      // Performs butterfly operation
      butterfly(local_sample, local_size, itr);

      // Gathers the values from each process
      MPI_Gather(local_sample, local_size, mpi_complex, global_sample, local_size, mpi_complex, 0, MPI_COMM_WORLD);
